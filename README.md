...
Our project is divided into 3 independent files.

**Timetable\_Data\_Wrangling.ipynb:** data wrangling on Timetable data. We filter the stops within 15km of Zurich, the route operating on weekdays and the trips running on the working hours of a day. Then we create dataframes for edges including walking edge and non-walking edge. 

**SBB\_Data\_Wrangling.ipynb:** Did similar data wrangling on actual data from SBB with what we did on Timetable data. And 
we Get the delay distributions for each stop at every given time and transportation type. If a transport arrives before its scheduled arrival time, the delay will be negative. Then we extract the hour from the timestamps and filter such that we only have times between 7AM to 5PM (which are business hours).

**Robust\_Journey\_Planning.ipynb:** This is the main notebook of the project which includes data loading, graph construction and route select algorithm. In addtion,it presents delay model and model to predict the success probability of each route. Then, it shows the visualization of "Journey Planning". Finally, we validate our route select algorithm and predictive model.

To test the model, firstly, you can run **Timetable\_Data\_Wrangling.ipynb** and **SBB\_Data\_Wrangling.ipynb** in PySpark kernel. In these two notebooks, we store the results on the HDFS in the folder under our username. Please make sure you have the access to rewrite the file in the folder, or you can store the results under your own folder. Also, you have another choice to load ours existing parquet data. In this way, you do not need to run the above two notebooks. Then you can run the **Robust\_Journey\_Planning.ipynb** in Python 3 kernel and input your test data through interactive visualization.

**Note:** **Robust\_Journey\_Planning.ipynb** is run on the local machine, we recommend you choose at least 2G memory.

**Here is the link to our introduction video**(https://youtu.be/WNb5L9o-t-4)
